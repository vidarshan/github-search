  ...
  const generateLanguageChartData = (repos) => {
    ...
    repos.map((repoLang, key) => {
      if (!langaugesByRepo.some((el) => el.name === repoLang.language)) {
        ...
        let indexOfLang = langaugesByRepo.findIndex((i) => i.name === repoLang.language)
        langaugesByRepo[indexOfLang].count = langaugesByRepo[indexOfLang].count + 1;
      }
    })
    ...
    langaugesByRepo.map((l, key) => {
      languageStats.push({
        id: key, name: l.name === null ? 'Misc' : l.name, value: Math.round((l.count / langCountTotal) * 100), color: colors.filter((lang) => {
    ...
    <Paper sx={{ minHeight: "100vh" }}>
      ...
      >
        ...
        <Group sx={{ marginTop: "1rem" }} direction="row" position="center">
          ...
          {userSearch.blog && <Anchor href={userSearch.blog} target="_blank">
            <ActionIcon
          ...
          {userSearch.twitter_username && <Anchor href={`https://www.twitter.com/${userSearch.twitter_username}`} target="_blank">
            <ActionIcon
          ...
          {userSearch.email && <Anchor href={`https://www.twitter.com/${userSearch.twitter_username}`} target="_blank">
            <ActionIcon
